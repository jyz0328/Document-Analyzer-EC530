...
import os
import textanalyzer_seperate

# Mock sys.argv for no input scenario
def test_no_input():
    with mock.patch('sys.argv', ['textanalyzer_seperate.py']):
        with pytest.raises(SystemExit) as e:
            textanalyzer_seperate.main()
        assert e.value.code == 1

# Mock for nonexistent file
def test_nonexistent_file():
    with mock.patch('sys.argv', ['textanalyzer_seperate.py', 'none.txt']):
...
def test_non_txt_file():
    with mock.patch('sys.argv', ['textanalyzer_seperate.py', 'grade.pdf']):
        with mock.patch('os.path.exists', return_value=True):
            with mock.patch('textanalyzer_seperate.analyze_document', return_value=3):
                with pytest.raises(SystemExit) as e:
                    textanalyzer_seperate.main()
...
def test_valid_txt_file():
    with mock.patch('sys.argv', ['textanalyzer_seperate.py', 'test.txt']):
        with mock.patch('os.path.exists', return_value=True):
            with mock.patch('textanalyzer_seperate.analyze_document', return_value=4):
                with pytest.raises(SystemExit) as e:
                    textanalyzer_seperate.main()
